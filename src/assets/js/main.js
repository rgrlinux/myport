...
function setupEventListeners() {
  ...
  if (elements.languageToggle && elements.languageMenu) {
    ...
    document.addEventListener("click", (e) => {
      if (
        !elements.languageToggle.contains(e.target) &&
  ...
  if (emailLink) {
    emailLink.addEventListener("click", (e) => {
      // Impede o navegador de seguir o '#'
      e.preventDefault();

      const user = "rgrlinux";
      const domain = "gmail.com";

      // Abre o cliente de e-mail diretamente via JS
...
function openLanguageMenu() {
  ...
  // Adicione a classe ao selector (pai) para o CSS .open funcionar
...
function setupObservers() {
  ...
  document
    .querySelectorAll(".feature-card, .service, .cta-card")
    .forEach((el) => {
...
function changeLanguage(lang) {
  ...
  elementsToTranslate.forEach((el) => {
    ...
    if (translations[lang] && translations[lang][key]) {
      ...
      console.warn(`Tradução não encontrada para: ${lang}.${key}`);
    }
  });
}
